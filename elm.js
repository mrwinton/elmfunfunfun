...
var author$project$Main$init = {buttonValue: author$project$Main$NoPlayer};
...
var author$project$Main$update = F2(
	function (msg, model) {
		return _Utils_eq(model.buttonValue, author$project$Main$X) ? _Utils_update(
			model,
			{buttonValue: author$project$Main$O}) : _Utils_update(
			model,
			{buttonValue: author$project$Main$X});
	});
var author$project$Main$Flip = {$: 'Flip'};
var author$project$Main$playerToString = function (p) {
	switch (p.$) {
		case 'X':
			return 'X';
		case 'O':
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						elm$html$Html$text(
						author$project$Main$playerToString(model.buttonValue))
					]))
			]));
};
